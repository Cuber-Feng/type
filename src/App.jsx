...
function TypingApp() {
  ...
  const backspacePercent = typed.length + backspaces ? ((backspaces / (typed.length + backspaces)) * 100).toFixed(1) : '0.0'
  ...
  useEffect(() => {
    ...
    const record = {
      ...
      accuracy,
      backspaces,
      backspacePercent: Number(backspacePercent),
      wpm,
    ...
    reader.onload = () => {
      try {
        ...
        const validRecords = imported.filter((record) => (
          ...
          backspaces: Number.isFinite(record.backspaces) ? record.backspaces : 0,
          backspacePercent: Number.isFinite(record.backspacePercent) ? record.backspacePercent : 0,
          wpm: record.wpm,
    ...
    <main className="typing-page">
      ...
      <section className="practice-area" aria-labelledby="page-title">
        ...
        <div className="test-panel">
          ...
          <div className="metrics" aria-live="polite"><div className="metric"><span>TIME</span><strong>{formatTime(elapsed)}</strong></div><div className="metric accuracy-metric"><span>ACCURACY | BACKSPACE</span><strong>{accuracy}% | {backspacePercent}%</strong></div><div className="metric speed-metric"><span>SPEED (WPM | CPM)</span><strong>{wpm} | {cpm}</strong></div></div>
        </div>
        <button className="restart-button" type="button" onClick={restart}><span aria-hidden="true">↻</span> <span>Restart (enter)</span></button>
              ...
              <div className="records-list">
                {records.map((record, index) => (
                  <div className="record-row" key={`${record.id}-${index}`}>
                    ...
                    <span className="record-accuracy"><b>{record.accuracy}% | {record.backspacePercent || 'N/A'}%</b> </span>
